...
<c:if test="${reloadmessage==1 }">
<script type="text/javascript">

$(function(){
		$('iframe').prop('src','checkseller');
});
</script>
</c:if><c:if test="${reloadmessage==2 }">
<script type="text/javascript">

$(function(){
		$('iframe').prop('src','checkorder');
});
</script>
</c:if><c:if test="${reloadmessage==3 }">
<script type="text/javascript">

$(function(){
		$('iframe').prop('src','checkgoods');
});
</script>
</c:if>
<c:if test="${reloadmessage==4 }">
<script type="text/javascript">

$(function(){
		$('iframe').prop('src','admincom');
});
</script>
</c:if>
		...
		<div class="sidebar" >
			...
			<ul class="list-unstyled list">
				...
				<li><a
					...
					class="anchorLink"><i class="icon-gift scolor"></i> 上架审核<c:if test="${agsum>0 }"><span class="unread">${agsum }</span></c:if></a></li>
				<li><a
				...
				<li><a
					onclick="javascript:document.getElementById('myiframe').src='shownews';"
					class="anchorLink"><i class="icon-info scolor"></i> 管理网站新闻</a></li>
