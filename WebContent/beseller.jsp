...
function validateForm(frmUpload) {
	...
	if (document.all.FileI.value == ""){
		alert("没有选择上传的文件！");
		frmUpload.FileI.focus();
		return false;
		}
	if (document.all.FileII.value == ""){
		alert("没有选择上传的文件！");
		frmUpload.FileII.focus();
		return false;
		}
	if (document.all.FileIII.value == ""){
		alert("没有选择上传的文件！");
		frmUpload.FileIII.focus();
		return false;
		}
	//截取提交上传文件的扩展名
	var ext1=frmUpload.FileI.value.match(/^(.*)(\.)(.{1,8})$/)[3];
	ext1=ext1.toLowerCase();
	var ext2=frmUpload.FileII.value.match(/^(.*)(\.)(.{1,8})$/)[3];
	ext2=ext2.toLowerCase();
	var ext3=frmUpload.FileIII.value.match(/^(.*)(\.)(.{1,8})$/)[3];
	ext3=ext3.toLowerCase();
	//设置允许上传文件的扩展名
	if ((ext1=="jpg"||ext1=="gif"||ext1=="png")||(ext2=="jpg"||ext2=="gif"||ext2=="png")||(ext3=="jpg"||ext3=="gif"||ext3=="png")){
		return true;
		}else {
...
<div class="login_sec">
	 <div class="container">
		 <ol class="breadcrumb">
		  <li class="active">升级为卖家</li>
		 </ol>
		 <c:if test="${sellerflag==0 }">你的资料还未完成审核，请耐心等待哦~<a href="userinfo">返回</a></c:if>
		 <c:if test="${sellerflag==1 }">你的资料未通过审核，理由是：${DisAgrMsg }请<a href="reupdate?uid=${user.uid }">点此</a>重新升级哦!</c:if>
		 <c:if test="${sellerflag==-1 }">
		 <h4>请依次您上传你的前身照片、身份证正面照片、身份证背面照片：</h4>
		 <div class="col-md-6 log">			 
				 <form name="frmUpload" enctype="multipart/form-data" action="sellerupload?uid=${user.uid }" method="post" onsubmit="return validateForm(this)">
					 <ul>
					 		<li class="text-info"><input type="file" name="FileI"></li>
					 		<li class="text-info"><input type="hidden" value=ext1 name="FileIContentType">
					 </ul>	
					 <ul>
					 		<li class="text-info"><input type="file" name="FileII"></li>
					 		<li class="text-info"><input type="hidden" value=ext2 name="FileIIContentType">
					 </ul>	
					 <ul>
					 		<li class="text-info"><input type="file" name="FileIII"></li>
					 		<li class="text-info"><input type="hidden" value=ext3 name="FileIIIContentType">
					 </ul>	
